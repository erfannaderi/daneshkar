...
class Order:
    ...
    def save_to_csv(self, filename):
        with open(filename, 'a', newline='') as file:
            ...
            writer.writerow([self.customer_code, self.product_name, self.month_number, self.base_price, self.quantity, self.final_amount])
...
orders = np.array([Order(customer_codes[i], product_names[i], month_numbers[i], base_prices[i], quantities[i]) for i in range(num_orders)])
